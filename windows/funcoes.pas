...
begin
  ...
  try
    Process.Executable := '/usr/bin/x-terminal-emulator'; // Caminho para o terminal padrão do sistema
    //Process.Executable := 'x-terminal-emulator'; // Caminho para o terminal padrão do sistema
    //Process.Parameters.Add('-e'); // Opção para executar um comando no terminal
    //Process.Parameters.Add('-t="Run Python"'); // Opção para executar um comando no terminal
    Process.Parameters.Add('-e'); // Opção para executar um comando no terminal
    //InputBox('teste','','/usr/bin/x-terminal-emulator'+' '+'-e'+FileName+' '+Source);
    if (Source= '') then
    begin
      Process.Parameters.Add(FileName ); // Adiciona o nome do programa e os argumentos
    end
    else
    begin
      Process.Parameters.Add(FileName);
      Process.Parameters.Add(Source); // Adiciona o nome do programa e os argumentos
    end;
    ...
    begin
         ...
         if (Process.Output.ReadByte <> 0) then
         begin
    ...
    if not Result then
      Output := 'Error executing program'
        ...
        Output := 'Success executing program';
